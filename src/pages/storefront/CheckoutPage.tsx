...
import { SupabaseNeighborhoodRepository } from '@/lib/infrastructure/repositories/SupabaseNeighborhoodRepository'
import { GetNeighborhoodsUseCase } from '@/lib/application/use-cases/GetNeighborhoodsUseCase'
...
export function CheckoutPage() {
  ...
  useEffect(() => {
    const repo = new SupabaseNeighborhoodRepository(supabase)
    new GetNeighborhoodsUseCase(repo).execute().then(setNeighborhoods)
  }, [])
  ...
  const fee = deliveryMethod === 'entrega' && selectedNeighborhood ? selectedNeighborhood.feeCents : 0
  const sub = subtotal()
            ...
            <Card className="p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {(['entrega', 'retirada'] as const).map((method) => (
                  ...
                  >
                    <input type="radio" value={method} {...register('deliveryMethod')} className="sr-only" />
              ...
              {deliveryMethod === 'entrega' ? (
                ...
                <div
                  ...
                  style={{ background: 'oklch(var(--c-surface-2))', color: 'oklch(var(--c-fg-soft))' }}
                >
                  <p className="font-medium mb-1" style={{ color: 'oklch(var(--c-fg))' }}>
                    Ateliê Delicias Ayumi
                  </p>
                  <p>R. Antônio Manoel de França, 187 Casa B · Corrêas, Petrópolis/RJ</p>
                ...
                {[
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-sm" style={{ color: 'oklch(var(--c-fg))' }}>{pm.label}</p>
                      <p className="text-xs" style={{ color: 'oklch(var(--c-fg-muted))' }}>{pm.sub}</p>
                    </div>
          ...
          <div className="md:sticky md:top-24 flex flex-col gap-4">
            <Card className="p-5">
              ...
              >
                ...
                {fee > 0 && (
                  <div className="flex justify-between">
                    <span style={{ color: 'oklch(var(--c-fg-soft))' }}>Taxa de entrega ({bairroValue})</span>
                ...
                >
                  ...
                  <span className="font-display text-2xl" style={{ color: 'oklch(var(--c-fg))' }}>
                    {brl(total)}
                  </span>
                </div>
              </div>
