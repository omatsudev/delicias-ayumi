    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-8">
      ...
      <div className="grid md:grid-cols-[1.1fr_1fr] gap-12">
        ...
        <div className="flex flex-col gap-3">
          ...
          {product.gallery_urls.length > 0 && (
            <div className="grid grid-cols-3 gap-2">
              {product.gallery_urls.slice(0, 3).map((url, i) => (
                <img
                  key={i}
                  src={url}
        ...
        <div className="flex flex-col gap-6">
          ...
          {!isOutOfStock ? (
            <div className="flex items-center gap-4">
              ...
              <Button size="lg" variant="primary" className="flex-1" onClick={handleAddToCart}>
                Adicionar à sacola · {brl(product.price_cents * qty)}
              </Button>
            </div>
          ) : (
            <div
          ...
          {/* WhatsApp */}
          <a
            href={`https://wa.me/${WA_NUMBER}?text=Olá! Tenho interesse no produto: ${product.name} (${brl(product.price_cents)})`}
            target="_blank"
